    ...
    <div>
        <div class="content">
            <div class="container-fluid">
                <p class="_title0">Events<button type="button" class="_add_btn" @click="showModal(Create,{after:loadEvents})">+ {{$t('add')}}</button></p>
                    ...
                    <div class="_overflow _table_div">
                        <table class="_table">
                            ...
                            <tr v-for="(event,index) in events" :key="index">
                                <td style="width: 54px;">{{event.id}}</td>
                                <td style="width: 200px;">{{event.name}}</td>
                                <td style="width: 400px;">{{event.place}}</td>
                                <td style="width: 200px;">{{event.date}}</td>
                                <td class="ddd" style="width: 300px;">
                                    <Button type="button" class="_btn _action_btn view_btn1" @click="showModal(Create,{info:true,data:event})">{{$t('view')}}</Button>
                                    <Button type="button" class="_btn _action_btn edit_btn1" @click="showModal(Create,{edit:true,data:event,after:loadEvents})">{{$t('edit')}}</Button>
                                    <Button type="button" class="_btn _action_btn make_btn1" @click="delete_it(event.id)">{{$t('delete')}}</Button>
                                </td>
                            </tr>
                            <!-- ITEMS -->
                        </table>
...
export default{
    ...
    methods:{
        ...
        delete_it(id){
            this.$http.get('events/delete?id='+id).then(response=>{
